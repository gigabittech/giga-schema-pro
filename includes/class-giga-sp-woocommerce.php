		...
		public static function get_wc_schema( $product_id ) {
			...
			if ( '' !== $price && ! $product->is_type('variable') ) {
				...
				foreach ( $available_variations as $variation ) {
					...
					// Add shipping and return policy to variant if global are set
					if ( ! empty( $settings['shippingRate'] ) ) {
			...
			if ( $gtin ) {
				$schema['gtin13'] = esc_attr( $gtin );
			}

			$mpn = get_post_meta( $product->get_id(), '_mpn', true );
			if ( $mpn ) {
				$schema['mpn'] = esc_attr( $mpn );
			}

			$brand = get_post_meta( $product->get_id(), '_brand', true );
			if ( $brand ) {
				$schema['brand'] = [ '@type' => 'Brand', 'name' => esc_attr( $brand ) ];
			}
